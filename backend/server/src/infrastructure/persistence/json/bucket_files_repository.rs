...
lazy_static::lazy_static! {
    pub static ref SHINNKU_FILES: Vec<FileInfo> = serde_json::from_str(SHINNKU_BUCKET_JSON)
        .expect("Failed to parse shinnku bucket files");

    pub static ref GALGAME0_FILES: Vec<FileInfo> = serde_json::from_str(GALGAME0_BUCKET_JSON)
        .expect("Failed to parse galgame0 bucket files");
}

pub fn filter_galgame0_files(files: &[FileInfo], prefix: &str) -> Vec<FileInfo> {
    files
        .iter()
